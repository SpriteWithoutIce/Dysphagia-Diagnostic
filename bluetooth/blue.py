...
class BluetoothConnection:
    ...
    def find_nearby_devices(self):
        ...
        try:
            ...
            while self.nearby_devices.__len__() == 0 and i < loop_num:
                ...
                time.sleep(2)
                print(
                    "No Bluetooth device around here! trying again {}...".format(str(i)))
            if not self.nearby_devices:
                print("There's no Bluetooth device around here. Program stop!")
            else:
                print("{} nearby Bluetooth device(s) has(have) been found:".format(
                    self.nearby_devices.__len__()), self.nearby_devices)  # 附近所有可连的蓝牙设备s
        ...
        if self.nearby_devices:
            for addr, name in self.nearby_devices:
                if target_name == name and target_address == addr:
                    print("Found target bluetooth device with address:{} name:{}".format(
    ...
    def connect_target_device(self, target_name, target_address):
        self.find_target_device(target_name=target_name,
...
if __name__ == '__main__':
    ...
    target_address = "C8:F0:9E:51:4D:A2"
    BluetoothConnection().connect_target_device(
